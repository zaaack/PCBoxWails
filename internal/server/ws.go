...
func (cm *ClientManager) Register(id, name string, ws *websocket.Conn) *Client {
	cm.mu.Lock()
	defer cm.mu.Unlock()
	client := &Client{ID: id, Name: name, Ws: ws, ConnectedAt: time.Now().UnixMilli()}
...
func (cm *ClientManager) GetAll() []ClientInfo {
	...
	for _, c := range cm.clients {
		result = append(result, ClientInfo{ID: c.ID, Name: c.Name, ConnectedAt: c.ConnectedAt})
	}
	return result
}
...
func NewWsServer(emitter EventEmitter) *WsServer {
	return &WsServer{
		emitEvent:     emitter,
...
func (s *WsServer) GetStatus() map[string]interface{} {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return map[string]interface{}{"running": s.running, "port": s.port}
}

func (s *WsServer) GetClients() []ClientInfo {
	return s.clientManager.GetAll()
}
...
func (s *WsServer) handleConnection(ws *websocket.Conn) {
	defer func() {
		...
		s.emitEvent("client-disconnected", nil)
		ws.Close()
	}()
	...
	for {
		...
		var msg WsMessage
		if err := jsonUnmarshal(message, &msg); err != nil {
...
func (s *WsServer) dispatch(msg WsMessage, ws *websocket.Conn) {
	...
	case 202, 204, 206, 208, 210, 212, 214, 218, 220, 222, 224, 226, 228, 230:
...
func (s *WsServer) handleRegister(msg WsMessage, ws *websocket.Conn) {
	...
	s.emitEvent("client-connected", map[string]interface{}{
		"id":   client.ID,
		"name": client.Name,
	})
...
func (s *WsServer) handleTopicResponse(msg WsMessage) {
	if msg.TopicID != nil {
		s.emitEvent("ws-response", map[string]interface{}{
			"topicId": *msg.TopicID,
			"code":    msg.Code,
			"data":    msg.Data,
		})
